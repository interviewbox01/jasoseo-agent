...
def main():
    ...
    with multiprocessing.Pool(processes=NUM_PROCESSES) as pool:
        with tqdm(total=NUM_TESTS, desc="가이드 생성 테스트") as pbar:
    ...
    html_template = """
    <!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>가이드 생성 테스트 보고서</title>
    ...
    </head><body><div class="container"><h1>가이드 생성 테스트 보고서</h1><div class="summary"><h2>요약</h2><p><strong>테스트 시간:</strong> {now}</p><p><strong>총 테스트 수:</strong> {total_tests}</p><p><strong>성공:</strong> {success_count}</p><p><strong>실패/에러:</strong> {failure_count}</p><p><strong>실패율:</strong> <span class="{status_class}">{failure_rate:.2f}%</span></p><p><strong>총 예상 비용:</strong> ${total_cost:.6f}</p></div>
